...
class UploadNotifier extends AsyncNotifier<TrackUploadMetadata> {
  ...
  Future<bool> submitTrack() async {
    ...
    if (currentState == null || currentState.audioFile == null) {
      return false;
    }
    ...
    ref.read(uploadSessionsProvider.notifier).startWatchingBeforeUpload(metadataToUpload.uploadId);
    ...
    return result.fold(
      ...
      (track) {
        ref.read(uploadSessionsProvider.notifier).trackUpload(
          uploadId: metadataToUpload.uploadId,
          track: track,
        );
